...
/* $Id: xgal.sndsrv.linux.c,v 1.3 1998/05/11 06:37:20 mrogre Exp $ */
#include <config.h>
...
void init (int argc, char **argv)
{
  ...
  if (argc != 3)
  {
    printf ("This program is only executed by xgalaga\n");
    exit (1);
  }
...
/*  dsp = open(dspdev, O_RDWR);  could not hear my oggs the same time */
/*  dsp = open(dspdev,  O_WRONLY | O_NONBLOCK); sound broken only fragmented*/
  dsp = open(dspdev, O_WRONLY); /* sound is ok*/
   if (dsp < 0)
  {
